...
namespace Harmonic.Controllers.Living
{
    public class LivingStream : NetStream
    {
            ...
            [FromOptionalArgument] bool reset = false)
        {
            ...
            Console.WriteLine($"Playing stream {streamName}.");
        ...
        public void Publish([FromOptionalArgument] string publishingName, [FromOptionalArgument] string publishingType)
        {
            ...
            //publishingType=live
            if (!streamingService.CheckStreamExists(publishingName))
            {
                throw new InvalidOperationException("invalid stream");
            }

            Console.WriteLine($"Incomming stream {publishingName} type {publishingType}.");
            var liveEncodingService = new LiveEncodingService();

            BackgroundJob.Enqueue(() => liveEncodingService.StartEncodingAsync(publishingName));
        ...
        protected override void Dispose(bool disposing)
        {
            if (!disposedValue)
            {
                try
                {
                    ...
                    var streamKey = _publisherSessionService.GetStreamKey(this);
                    Console.WriteLine($"Disconnecting stream  {streamKey}");
                }
                catch (Exception ex)
                {
                    Console.WriteLine($"Disconnecting stream  with error {ex.Message}");
                }
